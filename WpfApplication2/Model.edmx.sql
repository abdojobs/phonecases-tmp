...
-- --------------------------------------------------
-- Date Created: 04/29/2013 16:21:00
-- Generated from EDMX file: E:\ProgrammingProjects\PhoneCasesWPF\WpfApplication2\Model.edmx
-- --------------------------------------------------
...
IF OBJECT_ID(N'[pc].[FK_CompaniesLocations]', 'F') IS NOT NULL
    ALTER TABLE [pc].[Companies] DROP CONSTRAINT [FK_CompaniesLocations];
GO
IF OBJECT_ID(N'[pc].[FK_CustomersCompanies]', 'F') IS NOT NULL
    ALTER TABLE [pc].[Companies] DROP CONSTRAINT [FK_CustomersCompanies];
GO
IF OBJECT_ID(N'[pc].[FK_CompaniesCompanyType]', 'F') IS NOT NULL
    ALTER TABLE [pc].[Companies] DROP CONSTRAINT [FK_CompaniesCompanyType];
GO
...
CREATE TABLE [pc].[Cases] (
    ...
    [CloseTime] nvarchar(max)  NULL,
    [Reconnect] bit  NOT NULL,
    [HighPrio] bit  NOT NULL,
    [Closed] nvarchar(max)  NOT NULL,
    [Active] bit  NOT NULL,
...
-- Creating foreign key on [CustomerId] in table 'Cases'
ALTER TABLE [pc].[Cases]
